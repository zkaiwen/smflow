...
void AGGREGATION::simplify_output(AIG* aig, 
	...
	for(iMap = outInMap.rbegin(); iMap != outInMap.rend(); iMap++){
		printf("BEFORE Set %5d: ", iMap->first);
		std::set<unsigned>::iterator iPrint;
		for(iPrint = iMap->second.begin(); iPrint != iMap->second.end(); iPrint++)
			printf("%d ", *iPrint);
		printf("\n");
 ...
 *
 *#############################################################################*/
void AGGREGATION::adderAggregation3_verify(AIG* aig, 
		std::set<unsigned>& sumNodes,
...
void AGGREGATION::combineAdder(
	...
	//Combine based on Input similarity
	iListOut1 = addOut.begin();
	iListIn1 = addIn.begin();
	while(iListIn1 != addIn.end()){
		iListOut2 = iListOut1;
		iListOut2++;
		iListIn2 = iListIn1;
		iListIn2++;
...
void AGGREGATION::findMux2(CutFunction* cf, AIG* aigraph, std::map<unsigned,std::map<unsigned, unsigned> >& sizeCountMap){
	printf("\n\n\n");
